...
8. For local development, place the downloaded service-account JSON at `secrets/christ-hub-service-account.json` and set `GOOGLE_SERVICE_ACCOUNT_JSON_PATH=./secrets/christ-hub-service-account.json`. The `secrets` folder is git-ignored. Add the remaining values from `.env.local.example`.

Uploads go straight from the browser to Google Drive via a resumable-upload session (see Runtime routes below), so the account backing `drive` in `serviceClients()` needs its own Drive storage quota to receive files — a real Google account via `GOOGLE_REFRESH_TOKEN`/`GOOGLE_CLIENT_SECRET`/`GOOGLE_CLIENT_ID` (as already configured in production) works out of the box. A bare service account has no personal storage quota, so a service-account-only deployment needs the target Drive folder to live on a **Shared Drive** the service account can write to.
...
- `POST /api/broadcast/upload-session`: JSON body `{ idToken, fileName, mimeType, fileSize }`, used only when a post has media. Verifies the token, exact active directory membership, MIME type, and the 50 MB limit, then opens a Google Drive resumable-upload session and returns its URL. The browser then `PUT`s the file bytes straight to that URL — never through this app's own server — which is what lets a 50 MB photo or video actually reach 50 MB; routing the file through a Vercel function would hit its ~4.5 MB platform request-size limit first.
- `POST /api/broadcast/upload`: JSON body `{ idToken, caption, category, driveFileId?, mediaType? }`. Finalizes the post: makes any uploaded file link-viewable and writes the Sheets row. Root-domain and approved subdomain accounts are both supported when their exact email is listed in `OrgDirectory`. Omitting `driveFileId` publishes a text-only announcement (`mediaType: none`), which the story viewer renders as large styled text instead of a poster.
- `POST /api/broadcast/delete`: JSON body `{ postId, idToken }`. Re-verifies the GIS token, then only flips that row's `status` to `removed` (and out of the published feed) when the requester's email matches the post's `orgEmail` **and** the post is still within 5 minutes of its `timestamp`. This backs the story viewer's self-serve delete option, which the client only shows to the browser that just published the post, for 5 minutes.
- `POST /api/christ-hub/views`: JSON body `{ postId, viewerId }`, called as a heartbeat every few seconds while a story is open; returns the current "watching now" count for that post. `GET /api/christ-hub/views?postId=...` reads the same count without registering a heartbeat. Counts live only in server memory (no database), so they reset on redeploy and, on a multi-instance deployment, are only accurate per instance.
